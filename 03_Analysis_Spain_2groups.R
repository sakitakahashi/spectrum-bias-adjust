...
dat_sero_Spain %>%
	...
	ungroup() %>%
	filter(!str_detect(name, "Seroprevalence")) %>%
	tidyr::separate(name, c("tmp1","tmp2","name","metric")) %>%
	select(-tmp1, -tmp2) %>%
...
## Rename
Spain_df_2groups <- Spain_df
dat_sero_Spain_reported_2groups <- dat_sero_Spain_reported
dat_sero_Spain_adjusted_2groups <- dat_sero_Spain_adjusted
MAX_VALUE_SPAIN_2groups <- MAX_VALUE_SPAIN

ggplot() +
	geom_polygon(data=Spain_df_2groups %>% filter(lat >= 35), aes(x=long, y=lat, group=group, fill=adj_sero_1_median), color='black') +
	ggtitle("Spain: adjusted seroprevalence (Round 1)") +
	coord_map() +
	theme_bw() +
	theme( ## Box only
	...
	xlab("") + ylab("") +
	scale_fill_gradientn("Median", limits=c(0,MAX_VALUE_SPAIN_2groups), breaks=pretty_breaks(7), colours=brewer.pal(9, "YlOrRd")) -> p_Spain_1_adj_median_2groups

ggplot() +
	geom_polygon(data=Spain_df_2groups %>% filter(lat >= 35), aes(x=long, y=lat, group=group, fill=adj_sero_2_median), color='black') +
	ggtitle("Spain: adjusted seroprevalence (Round 2)") +
	coord_map() +
	theme_bw() +
	theme( ## Box only
	panel.grid.major = element_blank(),
	...
	axis.line=element_blank()) +
	xlab("") + ylab("") +
	scale_fill_gradientn("Median", limits=c(0,MAX_VALUE_SPAIN_2groups), breaks=pretty_breaks(7), colours=brewer.pal(9, "YlOrRd")) -> p_Spain_2_adj_median_2groups

dat_sero_Spain_reported_2groups %>%
	rename(reported=value) %>%
	left_join(dat_sero_Spain_adjusted_2groups, by=c("Area","Comunidades Autónomas","Provincia","ISO","name")) %>%
	rename(Round = name) %>%
	ggplot(aes(x=reported, y=median, colour=Round)) +
		...
		scale_y_continuous("Adjusted seroprevalence", breaks=scales::pretty_breaks(5)) -> p_Spain_scatter_2groups

(p_Spain_1_adj_median_2groups / p_Spain_2_adj_median_2groups) | p_Spain_scatter_2groups
...
dat_sero_Spain_adjusted_2groups %>%
...
dat_sero_Spain_reported_2groups %>%
	rename(reported=value) %>%
	left_join(dat_sero_Spain_adjusted_2groups, by=c("Area","Comunidades Autónomas","Provincia","ISO","name")) %>%
	rename(Round = name) -> dat_sero_Spain_2groups

## Save for future plotting etc
save(Spain_df_2groups, dat_sero_Spain_2groups, dat_sero_Spain_reported_2groups, dat_sero_Spain_adjusted_2groups, MAX_VALUE_SPAIN_2groups, dat_demog_Spain, dat_slopes, p_Spain_1_adj_median_2groups, p_Spain_2_adj_median_2groups, p_Spain_scatter_2groups, file="Results/Results_Spain_2groups.RData")
