...
weighted_Se_Abbott_1_under_15 <- gqs(model_weighted_Se, data=list(prop=c(sev_age_Manaus$prop_AS[1], 1-sev_age_Manaus$prop_AS[1]-sev_age_Manaus$prop_hosp[1], sev_age_Manaus$prop_hosp[1])), draws=as.matrix(fit_Stan_Abbott))

weighted_Se_Abbott_2_15_to_24 <- gqs(model_weighted_Se, data=list(prop=c(sev_age_Manaus$prop_AS[2], 1-sev_age_Manaus$prop_AS[2]-sev_age_Manaus$prop_hosp[2], sev_age_Manaus$prop_hosp[2])), draws=as.matrix(fit_Stan_Abbott))

weighted_Se_Abbott_3_25_to_34 <- gqs(model_weighted_Se, data=list(prop=c(sev_age_Manaus$prop_AS[3], 1-sev_age_Manaus$prop_AS[3]-sev_age_Manaus$prop_hosp[3], sev_age_Manaus$prop_hosp[3])), draws=as.matrix(fit_Stan_Abbott))

weighted_Se_Abbott_4_35_to_44 <- gqs(model_weighted_Se, data=list(prop=c(sev_age_Manaus$prop_AS[4], 1-sev_age_Manaus$prop_AS[4]-sev_age_Manaus$prop_hosp[4], sev_age_Manaus$prop_hosp[4])), draws=as.matrix(fit_Stan_Abbott))

weighted_Se_Abbott_5_45_to_54 <- gqs(model_weighted_Se, data=list(prop=c(sev_age_Manaus$prop_AS[5], 1-sev_age_Manaus$prop_AS[5]-sev_age_Manaus$prop_hosp[5], sev_age_Manaus$prop_hosp[5])), draws=as.matrix(fit_Stan_Abbott))

weighted_Se_Abbott_6_55_to_64 <- gqs(model_weighted_Se, data=list(prop=c(sev_age_Manaus$prop_AS[6], 1-sev_age_Manaus$prop_AS[6]-sev_age_Manaus$prop_hosp[6], sev_age_Manaus$prop_hosp[6])), draws=as.matrix(fit_Stan_Abbott))

weighted_Se_Abbott_7_65_to_70 <- gqs(model_weighted_Se, data=list(prop=c(sev_age_Manaus$prop_AS[7], 1-sev_age_Manaus$prop_AS[7]-sev_age_Manaus$prop_hosp[7], sev_age_Manaus$prop_hosp[7])), draws=as.matrix(fit_Stan_Abbott))
...
dat_sero_Manaus_AGG %>%
	ggplot() +
		...
		scale_x_date(date_labels="%b %Y", date_breaks="1 month") +
		xlab("") +
		ggtitle("Manaus, Brazil: adjusted seroprevalence over time - with AS as separate category, across ages 15-70") +
...
## Prep to save
dat_sero_Manaus_3groups <- dat_sero_Manaus
dat_sero_Manaus_AGG_3groups <- dat_sero_Manaus_AGG
p_Manaus_ts_3groups <- p_Manaus_ts
p_Manaus_scatter_3groups <- p_Manaus_scatter

## Save for future plotting etc
save(
	dat_sero_Manaus_3groups,
	dat_sero_Manaus_AGG_3groups,
	p_Manaus_ts_3groups,
	p_Manaus_scatter_3groups,
	dat_slopes,
	as.Date_origin,
	file="Results/Results_Manaus_3groups.RData")
