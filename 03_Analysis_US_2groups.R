...
for(i in 1:nrow(dat_sero_US_track)) {
	...
	weighted_Se_Abbott_1_0_to_17 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[1], prop_hosp[1])), draws=as.matrix(fit_Stan_Abbott))
	weighted_Se_Abbott_2_18_to_49 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[2], prop_hosp[2])), draws=as.matrix(fit_Stan_Abbott))
	weighted_Se_Abbott_3_50_to_64 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[3], prop_hosp[3])), draws=as.matrix(fit_Stan_Abbott))
	weighted_Se_Abbott_4_65_plus <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[4], prop_hosp[4])), draws=as.matrix(fit_Stan_Abbott))

	weighted_Se_Roche_1_0_to_17 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[1], prop_hosp[1])), draws=as.matrix(fit_Stan_Roche))
	weighted_Se_Roche_2_18_to_49 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[2], prop_hosp[2])), draws=as.matrix(fit_Stan_Roche))
	weighted_Se_Roche_3_50_to_64 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[3], prop_hosp[3])), draws=as.matrix(fit_Stan_Roche))
	weighted_Se_Roche_4_65_plus <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[4], prop_hosp[4])), draws=as.matrix(fit_Stan_Roche))
	
	weighted_Se_VitrosIgG_1_0_to_17 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[1], prop_hosp[1])), draws=as.matrix(fit_Stan_VitrosIgG))
	weighted_Se_VitrosIgG_2_18_to_49 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[2], prop_hosp[2])), draws=as.matrix(fit_Stan_VitrosIgG))
	weighted_Se_VitrosIgG_3_50_to_64 <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[3], prop_hosp[3])), draws=as.matrix(fit_Stan_VitrosIgG))
	weighted_Se_VitrosIgG_4_65_plus <- gqs(model_weighted_Se, data=list(prop=c(1-prop_hosp[4], prop_hosp[4])), draws=as.matrix(fit_Stan_VitrosIgG))
...
dat_sero_US_track %>%
	ggplot(aes(x=seroprevalence_raw, y=adj_sero_median, colour=Testing_Period)) +
		...
		geom_abline(dat=dat_slopes, aes(intercept=intercept, slope=slope, linetype=slope_factor), size=0.5) +
		scale_linetype_manual(name = "Bias", values=rev(c(1,5,6,3))) +
		geom_point() +
...
ggplot(as.data.frame(survey_4_weighted_by_age_and_sex_final), aes(map_id=state)) +
  ...
  expand_limits(x = fifty_states$long, y = fifty_states$lat) +
	ggtitle("United States: adjusted seroprevalence (Round 4)") +
	theme_bw() +
	theme( ## Box only
...
## Prep to save
dat_sero_US_track_2groups <- dat_sero_US_track
dat_US_state_counts_2groups <- dat_US_state_counts
survey_4_weighted_by_age_and_sex_final_2groups <- survey_4_weighted_by_age_and_sex_final
p_US_adj_4_2groups <- p_US_adj_4
p_US_scatter_4_2groups <- p_US_scatter_4
MAX_VALUE_US_2groups <- MAX_VALUE_US
CD_1_2groups <- CD_1
CD_2_2groups <- CD_2
CD_3_2groups <- CD_3
CD_4_2groups <- CD_4
...
CD_8_2groups <- CD_8
CD_9_2groups <- CD_9
PR_spdf_2groups <- PR_spdf

## Save for future plotting etc
save(
	dat_sero_US_track_2groups,
	dat_US_state_counts_2groups,
	survey_4_weighted_by_age_and_sex_final_2groups,
	p_US_adj_4_2groups,
	p_US_scatter_4_2groups,
	MAX_VALUE_US_2groups,
	CD_1_2groups, CD_2_2groups, CD_3_2groups,
	CD_4_2groups, CD_5_2groups, CD_6_2groups,
	CD_7_2groups, CD_8_2groups, CD_9_2groups,
	PR_spdf_2groups, dat_slopes,
	file="Results/Results_US_2groups.RData")
