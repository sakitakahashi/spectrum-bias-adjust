...
parameters {
	...
	vector[J_3] log_intercept_raw_3;
	
	real<lower=0.0> sigma;
	
	positive_ordered[2] tau_backwards;		// Order: high, low
	
	real<lower=0.0, upper=1.0> theta[3]; 	// Probability of being a low (theta) or high (1-theta) responder in group 2
...
model {
	...
	for (j in 1:J_3) {
			...
			target += log_mix(theta[3],
				normal_lpdf(log_intercept_raw_3[j] | log_beta_severity[1], tau[1]),
				normal_lpdf(log_intercept_raw_3[j] | log_beta_severity[2], tau[2]));
	...
	for (n in 1:N) {
		...
		if(severity_by_obs[n]==3) {
			...
			if(N_UNIQUE_LAMBDA==1) target += normal_lpdf(log_SC_value[n] | log_intercept_raw_3[patient_ID_index_by_severity[n]] - (lambda[1] * time_since[n]), sigma);
			
			else if(N_UNIQUE_LAMBDA==2) {
				
				target += log_mix(theta[3],
					normal_lpdf(log_SC_value[n] | log_intercept_raw_3[patient_ID_index_by_severity[n]] - (lambda[1] * time_since[n]), sigma),
					normal_lpdf(log_SC_value[n] | log_intercept_raw_3[patient_ID_index_by_severity[n]] - (lambda[2] * time_since[n]), sigma));
	...
	target += normal_lpdf(theta[3] | 0.5, 0.5);
	
}
generated quantities {
	
	matrix<lower=0.0, upper=1.0>[3,(DAYS_FOR_SENS+1)] sensitivity_over_time;	// Sensitivity: 3 classes of severity (AS, S & non-hosp, hosp)
	...
	for(k in 0:DAYS_FOR_SENS) {
		...
		vector[N_rep] which_group_3_low;
		...
		for(x in 1:N_rep) {
			...
			which_group_3_low[x] = bernoulli_rng(theta[3]);
			...
			if(N_UNIQUE_LAMBDA==1) {
				...
				Y_tilde_3[x] = which_group_3_low[x] == 1 ?
						log_beta_severity[1] + intercept_new_1_tau[x] + intercept_new_sigma[x] - (lambda[1] * (k * 1.0)) :
						log_beta_severity[2] + intercept_new_2_tau[x] + intercept_new_sigma[x] - (lambda[1] * (k * 1.0));
			...
			Y_tilde_binary_3[x] = Y_tilde_3[x] >= CUTOFF ? 1.0 : 0.0;
		...
		sensitivity_over_time[3,k+1] = sum(Y_tilde_binary_3) / (1.0 * N_rep);
	...
	for (n in 1:N) {
		...
		if(severity_by_obs[n]==3) {
			...
			if(N_UNIQUE_LAMBDA==1) log_lik[n] = normal_lpdf(log_SC_value[n] | log_intercept_raw_3[patient_ID_index_by_severity[n]] - (lambda[1] * time_since[n]), sigma);
			
			else if(N_UNIQUE_LAMBDA==2) {
				
				log_lik[n] = log_mix(theta[3],
					normal_lpdf(log_SC_value[n] | log_intercept_raw_3[patient_ID_index_by_severity[n]] - (lambda[1] * time_since[n]), sigma),
					normal_lpdf(log_SC_value[n] | log_intercept_raw_3[patient_ID_index_by_severity[n]] - (lambda[2] * time_since[n]), sigma));
	...
	for(j in 1:J) {
		...
		for(k in 0:DAYS_FOR_SENS) {
			...
			if(severity_by_ind[j]==3) log_SC_value_fitted[j,k+1] = normal_rng(log_intercept_raw_3[patient_ID_index_by_severity_by_ind[j]] - (lambda[1] * (k * 1.0)), sigma);
