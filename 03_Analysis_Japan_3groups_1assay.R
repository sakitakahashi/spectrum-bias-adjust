...
weighted_Se_Roche <- gqs(model_weighted_Se, data=list(prop=c(prop_AS, 1-prop_AS-prop_hosp, prop_hosp)), draws=as.matrix(fit_Stan_Roche))
...
## Prep to save
dat_sero_Japan_univariate_3groups <- dat_sero_Japan_univariate

# Save for future plotting etc
save(
	dat_sero_Japan_univariate_3groups,
	file="Results/Results_Japan_3groups_1assay.RData")
