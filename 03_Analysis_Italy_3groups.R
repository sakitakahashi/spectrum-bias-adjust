...
## Rename
Italy_df_3groups <- Italy_df
dat_sero_Italy_3groups <- dat_sero_Italy
MAX_VALUE_ITALY_3groups <- MAX_VALUE_ITALY

ggplot() +
	geom_polygon(data=Italy_df_3groups, aes(x=long, y=lat, group=group, fill=adj_sero_median), color='black') +
	ggtitle("Italy: adjusted seroprevalence - with AS as separate category") +
	coord_map() +
	theme_bw() +
	theme( ## Box only
	...
	xlab("") + ylab("") +
	scale_fill_gradientn("Median", limits=c(0,MAX_VALUE_ITALY_3groups), breaks=pretty_breaks(7), colours=brewer.pal(9, "YlOrRd")) -> p_Italy_adj_median_3groups
...
dat_sero_Italy_3groups %>%
	ggplot(aes(x=Seroprevalence, y=adj_sero_median)) +
		...
		scale_y_continuous("Adjusted seroprevalence", breaks=scales::pretty_breaks(5)) -> p_Italy_scatter_3groups

p_Italy_adj_median_3groups / p_Italy_scatter_3groups
...
save(Italy_df_3groups, dat_sero_Italy_3groups, MAX_VALUE_ITALY_3groups, dat_slopes, p_Italy_adj_median_3groups, p_Italy_scatter_3groups, file="Results/Results_Italy_3groups.RData")
