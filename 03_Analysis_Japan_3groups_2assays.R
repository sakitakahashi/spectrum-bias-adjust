...
FILENAME <- "Results/Stan_fit_bivariate_3groups.RData"
load(FILENAME)
...
as.data.frame(summary(fit_Stan)$summary) %>%
	...
	mutate(severity_final_class = factor(severity_final_class, levels=c(1:12), labels=c("AS (+,+)", "AS (+,-)", "AS (-,+)", "AS (-,-)", "S,non-hosp (+,+)", "S,non-hosp (+,-)", "S,non-hosp (-,+)", "S,non-hosp (-,-)", "S,hosp (+,+)", "S,hosp (+,-)", "S,hosp (-,+)", "S,hosp (-,-)"))) %>%
	ggplot() +
		theme_bw() +
...
calc_wtd_Se_by_time_adjusting_for_severity_3groups_bivariate <-'
data {
...
parameters {
	...
	matrix<lower=0.0, upper=1.0>[12,366] sensitivity_over_time;
...
generated quantities {
	...
	for(i in 1:366) {
		...
		sensitivity_wtd[1,i] = (prop[1]*sensitivity_over_time[1,i]) + (prop[2]*sensitivity_over_time[5,i]) + (prop[3]*sensitivity_over_time[9,i]);
		sensitivity_wtd[2,i] = (prop[1]*sensitivity_over_time[2,i]) + (prop[2]*sensitivity_over_time[6,i]) + (prop[3]*sensitivity_over_time[10,i]);
		sensitivity_wtd[3,i] = (prop[1]*sensitivity_over_time[3,i]) + (prop[2]*sensitivity_over_time[7,i]) + (prop[3]*sensitivity_over_time[11,i]);
		sensitivity_wtd[4,i] = (prop[1]*sensitivity_over_time[4,i]) + (prop[2]*sensitivity_over_time[8,i]) + (prop[3]*sensitivity_over_time[12,i]);
		
	}
}
'

model_weighted_Se <- stan_model(model_code=calc_wtd_Se_by_time_adjusting_for_severity_3groups_bivariate)

## Generate severity-weighted sensitivities
## This is ordered as (Abbott, Roche): (+,+), (+,-), (-,+), (-,-)
weighted_Se <- gqs(model_weighted_Se, data=list(prop=c(prop_AS, 1-prop_AS-prop_hosp, prop_hosp)), draws=as.matrix(fit_Stan))
...
fit_Stan_cond_covariance <- stan(
	...
	data=list(
	## This is ordered as (Abbott, Roche): (+,+), (+,-), (-,+), (-,-)
	# Se = c(mu_Se_1, mu_Se_2, mu_Se_3, mu_Se_4),
...
dat_sero_Japan_bivariate %>%
	...
	ggplot() +
		...
		theme_bw() +
		xlab("") +
		ylab("Seroprevalence - with AS as separate category") +
		facet_wrap(.~Prefecture, scales="fixed", ncol=5)

## Prep to save
dat_sero_Japan_bivariate_3groups <- dat_sero_Japan_bivariate

# Save for future plotting etc
save(
	dat_sero_Japan_bivariate_3groups,
	file="Results/Results_Japan_3groups_2assays.RData")
